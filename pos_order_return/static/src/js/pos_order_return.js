...
odoo.define('pos_order_return.pos_order_return', function(require) {
    ...
    var ReceiptAllScreenWidgetWK = screens.ScreenWidget.extend({
       template: 'ReceiptAllScreenWidgetWK',
        show: function() {
            var self = this;
            self._super();
    ...
    gui.define_screen({ name: 'ReceiptAllScreenWidgetWK', widget: ReceiptAllScreenWidgetWK });
    ...
    pos_orders.include({
        ...
        display_order_details: function(visibility, order, clickpos) {
            ...
            if (visibility === 'show') {
                ...
                this.$("#wk_reprint").on("click", function() {
                    var order_id = order.id;
                    var orderlines = [];
                    var customer = self.pos.db.get_partner_by_id(order.partner_id[0]);
                    var paymentlines = [];
                    var canvas = document.createElement('canvas');
                    JsBarcode(canvas, order.pos_reference);
                    var barcode_img = canvas.toDataURL("image/png");
                    var discount = 0;
                    var subtotal = 0;
                    var tax = 0;
                    var tax_total = 0;
                    var total_qty = 0;
                    var shop_image = 'data:image/png;base64,'+ self.pos.company.pos_logo;
                    var shop_address = {}
                    if (self.pos.pos_shops) {
                        _.each(self.pos.pos_shops, function(shop){
                    ...
                    rpc.query({
                        ...
                        self.gui.show_screen('ReceiptAllScreenWidgetWK');
                        $('.pos-receipt-container').html(QWeb.render('OrderReceipt1',rslt));
                        if (output.length === 11 && output[10]) {
                            var invoice_number = output[10];
                            $('.pos-receipt-container #invoice_number').text(invoice_number);
                            jQuery(".pos-receipt-container #qr-example").qrcode({
                                width: 120,
                                height: 120,
                                text: invoice_number,
                            });
