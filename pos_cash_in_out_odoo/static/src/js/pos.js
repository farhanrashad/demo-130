...
odoo.define('pos_cash_in_out_odoo.pos', function(require) {
	...
	models.load_models({
		...
		fields: ['name'],
		domain: [],
		loaded: function(self, pos_reason){
	...
	var CashOutPopupWidget = popups.extend({
		...
		cash_out: function() {
			...
			var entered_reason = $("#pos_reason_out").val();
			var reason_text = $("#pos_reason_out").children("option:selected").text();
			var remark_out = $("textarea#remark_out").val();
			...
			else if(entered_reason == '' || entered_reason === '0')
			{
				$("#error").text("Please select reason to withdraw");
				$('#error').show();
				setTimeout(function() {$('#error').hide()},2000);
				return;
			}
				...
				rpc.query({
					...
					args: [cashier ? cashier.id : 0, cashier ? cashier.id : 0, entered_reason, entered_amount, session_id, remark_out],

				}).then(function(output) {
					if (output == true){
						self.save_summary_details('Take Money Out', reason_text,entered_amount)
	...
	var CashInPopupWidget = popups.extend({
		...
		{	
			...
			var selectedOrder = self.pos.get('selectedOrder');
			var cashier = self.pos.cashier || self.pos.user;
			var entered_reason = $("#pos_reason_in").val();
			var reason_text = $("#pos_reason_in").children("option:selected").text();
			var remark_in = $("textarea#remark_in").val();
			...
			else if(entered_reason == '' || entered_reason === '0')
			{
				...
				rpc.query({
					...
					args: [cashier ? cashier.id : 0, cashier ? cashier.id : 0, entered_reason, entered_amount, session_id, remark_in],

				}).then(function(output) {
					if (output == true){
						self.save_summary_details('Put Money In', reason_text,entered_amount)
