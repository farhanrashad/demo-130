...
class GenerateXLSXReport(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.set_column(13, 20, 50)

        config_ids = self.env['pos.order'].search([('id', 'in', data['config_ids'])])

        for id in config_ids:
            config_name = self.env['pos.details.wizard'].browse(id)
            for line in id.lines:
                #                 one_style_short_value = ''
                #                 fabric_design_short_value = ''
                #                 five_style_short_value = ''
                #                 fabric_code_short_value = ''
                #                 fabric_comp_short_value = ''
                #                 fabric_color_short_value = ''
                #                 season_short_value = ''
                #                 order_type_short_value = ''
                ...
                values = []
                for val in line.product_id.attribute_line_ids:
                    values.append(val.value_ids.name)
                value = ' '.join(values)

                sheet.write(row, 0, config_name.pos_config_ids.name, format2)
                #sheet.write(row, 1, id.employee_id.name, format2)
                sheet.write(row, 2, invoice_date, format2)
                sheet.write(row, 3, category, format2)
                sheet.write(row, 4, id.pos_reference, format2)
                sheet.write(row, 5, id.payment_ids.amount, format2)
                sheet.write(row, 6, line.discount, format2)
                sheet.write(row, 7, line.price_subtotal_incl, format2)
                sheet.write(row, 8, id.payment_ids.payment_method_id.name, format2)
                sheet.write(row, 9, id.partner_id.name, format2)
                sheet.write(row, 10, id.partner_id.phone, format2)
