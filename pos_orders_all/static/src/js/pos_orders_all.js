odoo.define('pos_orders_all.pos', function(require) {
	...
	models.load_models({
		model: 'pos.order',
		fields: ['name', 'id', 'return_id', 'date_order','discount_type', 'partner_id', 'pos_reference', 'lines', 'amount_total','amount_tax','session_id', 'state', 'company_id','coupon_id','barcode','barcode_img'],
		domain: function(self){ 
			var current = self.pos_session.id
			if (self.config.pos_session_limit_all == 'all')
			{
				return [['state', 'not in', ['draft', 'cancel']]]; 
			}
			if (self.config.pos_session_limit_all == 'last3')
			{
				return [['state', 'not in', ['draft', 'cancel']],['session_id', 'in',[current,current-1,current-2,current-3]]]; 
			}
			if (self.config.pos_session_limit_all == 'last5')
			{
	...
	models.load_models({
		...
		fields: ['order_id', 'product_id', 'discount', 'qty', 'return_qty', 'price_unit','discount_line_type'],
		domain: function(self) {
			var order_lines = []
	...
	screens.ProductListWidget.include({
		init: function(parent, options) {
			...
			this.click_product_handler = function(){
				...
				var warning = "You have selected an item which has Zero or Negative Stock On Hand";
	...
	models.Order = models.Order.extend({
		...
		export_as_JSON: function() {
			...
			json.barcode_img = this.barcode_img;
	...
	// End POSBarcodeReturnWidget       
	 ...
	 // ReceiptAllScreenWidgetNew start
	 var ReceiptAllScreenWidgetNew = screens.ScreenWidget.extend({
	   template: 'ReceiptAllScreenWidgetNew',
		show: function() {
			var self = this;
			self._super();
	...
	gui.define_screen({ name: 'ReceiptAllScreenWidgetNew', widget: ReceiptAllScreenWidgetNew });
	...
	var SeeAllOrdersScreenWidget = screens.ScreenWidget.extend({
		...
		set_return_order: function(order) {
			rpc.query({
					model: 'pos.order',
					method: 'get_return_data',
					args: [order.id, order.name],
				
				}).then(function(output) {
				   order['return_id'] = output;
			});
		},
		set_return_qty: function(order) {
			rpc.query({
					model: 'pos.order',
					method: 'set_return_qty',
					args: [order.id, order.name],
				}).then(function(qty) {
				   order['return_qty'] = qty || 0;
			});
		},
		render_list_orders: function(orders, search_input){
		   ...
		   if (search_input != undefined && search_input != '') {
			...
			this.$el.find('.order-details').css("display", "none");;
			content.innerHTML = "";
			var orders = orders;
			var pos_lines = this.pos.pos_order_line;
			for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
				var order = orders[i];
				var total_qty = 0;
				for (var j = 0; j < pos_lines.length; j++) {
					if (pos_lines[j].order_id[0] == order.id) {
						total_qty += pos_lines[j].qty 
					}
				}
				if (!("return_id" in order)){
					self.set_return_order(order);
				}
				/*if (order.return_id == false){
					self.set_return_qty(order);
				}*/
		...
		line_select: function(event,$line,id) {
			this.$('.orders-list-contents').find('.highlight').removeClass('highlight');
			this.$($line).parent().addClass('highlight');
			this.display_order_details(id)
	    },
	    get_tax_lines: function(order) {
	    	var self = this;
	    	var def;
	    	def = rpc.query({
					model: 'pos.order',
					method: 'get_tax_by_lines',
					args: [order.id],
				
				});
	    	return def.then(function(output) {
	    		return $.when(output);
			});
	    },
	    display_order_details: function(order_id){
	        var self = this;
	        var contents = this.$('.client-details-contents');
	        ...
	        for(var ord = 0; ord < orders.length; ord++){
				   if (orders[ord]['id'] == order_id){
					var orders1 = orders[ord];
				   }
			   	}
	   			self.get_tax_lines(orders1).then(function(res){
				   	var orderline = [];
			   		_.each(order_lines, function(line){
			   			if (line['order_id'][0] == order_id){
					   		_.each(res, function(tax_line){
					   			if (tax_line[0] == line.id) {
					   				line['tax'] = tax_line[1];
					   			}
					   		});
					   		line['total'] = (line.qty * line.price_unit) - line.discount + line['tax']
							orderline.push(line);
					   	}
			   		})
            		contents.empty();
            		contents.append($(QWeb.render('LineDetails',{widget: self, 'order': orders1, 'orderline':orderline})));
            	});
	    },
		show: function(options) {
			var self = this;
			...
			this.$('.orders-list-contents').on('click', '.orders-line', function(event){
			...
			this.$('.orders-list-contents').delegate('.print-order', 'click', function(result) {
				...
				rpc.query({
					...
					var rslt = {
						...
						client: output[7],
						client_phone: output[8],
					}
					self.gui.show_screen('ReceiptAllScreenWidgetNew');
					$('.pos-receipt-container').html(QWeb.render('OrderReceipt1',rslt));
			...
			//Return Order
			this.$('.client-details-contents').delegate('.return-order', 'click', function(result) {
				var order_id = parseInt(this.id);
				var selectedOrder = null;
				for(var i = 0, len = Math.min(orders.length,1000); i < len; i++) {
					if (orders[i] && orders[i].id == order_id) {
			...
			this.$('.client-details-contents').delegate('.re-order', 'click', function(result) {				
				var order_id = parseInt(this.id);
