...
class pos_order(models.Model):
    ...
    def create(self, vals):
        order_id = super(pos_order, self).create(vals)
    ...
    @api.model
    def get_tax_by_lines(self, order):
        order = self.env['pos.order'].browse(order)
        tax_lines = []
        for line in order.lines:
            tax_lines.append((line.id, self._amount_line_tax(line, order.fiscal_position_id)))
        return tax_lines
    ...
    def print_pos_receipt(self):
        ...
        for orderline in orderlines:
            ...
            output.append(new_vals)
        return [output, discount, paymentlines, change, subtotal, tax, barcode_img, client, client_phone]
...
class pos_config(models.Model):
    ...
    pos_session_limit_all = fields.Selection([('all',  "Load all Session's Orders"), ('last3', "Load last 3 Session's Orders"), ('last5', " Load last 5 Session's Orders")], string='Session limit')
