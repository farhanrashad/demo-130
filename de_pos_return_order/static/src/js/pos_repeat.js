    ...
    render_list:function(options){
        ...
        rpc.query({
                ...
                for(var j=0;j < lines.length; j++){
                    ...
                    list_qty.push (product_line.qty)
    ...
    click_confirm: function(){
        ...
        for (r=0, n = myTable.rows.length; r < n; r++) {
            ...
            for (var r=0, n = myTable.rows.length; r < n; r++) {
                ...
                for (var s=0; s < list_qty.length; s++){
                    if (ordered_qty > 0 && ordered_qty <= list_qty[s]){
                        // alert('Total: ' + list_qty + ' Ordered: ' + ordered_qty);
                        var current_values = {
                        ...
                        this.pos.get_order().add_product(product, current_values);
                    }
                    else if (ordered_qty == 0){
                        count = count + 1
                        if (list_qty.length == count){
                            alert("You are not allowed to return all zero quantities");
                        }
                    }
                    else{
                        alert("Please Enter Valid Quantity for " + product.display_name);
                    }
                    break;
