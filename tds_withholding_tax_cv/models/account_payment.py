...
class account_payment(models.Model):
    ...
    tds_type = fields.Selection([('default', 'Payment Without WHT'), ('including', 'Payment Including WHT'), ('excluding', 'Payment Excluding WHT')], default="default", string="WHT Type")
    tds_tax_id = fields.Many2one('account.tax', string='WHT')
    tds_amt = fields.Float(string='WHT Amount', compute='compute_tds_amnt')
    ...
    def set_default_tds_tax_id(self):
        if self.partner_id.wht_id and self.tds_type in ('including', 'excluding'):
    ...
    def compute_tds_amnt(self):
        for payment in self:
            ...
            if payment.tds_type in ('including', 'excluding') and payment.tds_tax_id and payment.amount and payment.payment_type == 'outbound':
                ...
                if applicable and payment.tds_type == 'excluding':
                    #payment.tds_amt = (payment.tds_tax_id.amount * payment.amount / 100)
                    payment.tds_amt = payment.amount / (100 - payment.tds_tax_id.amount) * payment.tds_tax_id.amount
                elif applicable and payment.tds_type == 'including':
                    payment.tds_amt = (payment.tds_tax_id.amount * payment.amount / 100)
                else:
                    payment.tds_amt = 0.0
    ...
    def _prepare_payment_moves(self):
        ...
        for payment in self:
            ...
            if payment.tds_type in ('including', 'excluding') and payment.tds_tax_id and payment.tds_amt and applicable:
