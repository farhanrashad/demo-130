...
class MakeInvoice(models.TransientModel):
    _name = 'repair.order.make_invoice.operation'
    _description = 'Create Operation Invoice (repair)'

    group = fields.Boolean('Group by partner operation invoice address')

    def make_operation_invoices(self):
        if not self._context.get('active_ids'):
            return {'type': 'ir.actions.act_window_close'}
        new_invoice = {}
        for wizard in self:
            repairs = self.env['repair.order'].browse(self._context['active_ids'])
            new_invoice = repairs._create_invoices_operation(group=wizard.group)
            ...
            repairs.action_repair_operations_invoice_create()
        return {
