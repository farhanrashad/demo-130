...
class MakeInvoice(models.TransientModel):
    _name= 'repair.order.make_invoice.part'
    _description = 'Create Part Invoice (repair)'

    group = fields.Boolean('Group by partner part invoice address')

    def make_part_invoices(self):
        if not self._context.get('active_ids'):
            return {'type': 'ir.actions.act_window_close'}
        new_invoice = {}
        for wizard in self:
            repairs = self.env['repair.order'].browse(self._context['active_ids'])
            new_invoice = repairs._create_invoices_part(group=wizard.group)
            ...
            repairs.action_repair_part_invoice_create()
        return {
