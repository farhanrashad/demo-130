...
class AccountMove(models.Model):
    ...
    repair_id = fields.Many2one('repair.order', store=False, readonly=True,
        states={'draft': [('readonly', False)]},
        string='Repair Order',
        help="Auto-complete from a past purchase order.")

    @api.onchange('purchase_vendor_bill_id', 'repair_id')
    def _onchange_purchase_auto_complete(self):
        ...
        * If it's a purchase order, 'purchase_id' is set and this method will load lines.
        ...
        if self.purchase_vendor_bill_id.vendor_bill_id:
            ...
            self.repair_id = self.purchase_vendor_bill_id.purchase_order_id
        ...
        # Copy purchase lines.
        po_lines = self.purchase_id.order_line - self.line_ids.mapped('repair_line_id')
        ...
        # Compute ref.
        refs = set(self.line_ids.mapped('repair_line_id.repair_id.partner_ref'))
    ...
    def create(self, vals_list):
        ...
        for move in moves:
            if move.reversed_entry_id:
                continue
            purchase = move.line_ids.mapped('repair_line_id.repair_id')
            if not purchase:
                continue
    ...
    def write(self, vals):
        # OVERRIDE
        old_purchases = [move.mapped('line_ids.repair_line_id.repair_id') for move in self]
        res = super(AccountMove, self).write(vals)
        for i, move in enumerate(self):
            new_purchases = move.mapped('line_ids.repair_line_id.repair_id')
            if not new_purchases:
                continue
            diff_purchases = new_purchases - old_purchases[i]
            if diff_purchases:
                refs = ["<a href=# data-oe-model=repair.order data-oe-id=%s>%s</a>" % tuple(name_get) for name_get in diff_purchases.name_get()]
...
class AccountMoveLine(models.Model):
    ...
    repair_line_id = fields.Many2one('repair.line', 'Repair Order Line', ondelete='set null', index=True)

    def _copy_data_extend_business_fields(self, values):
        # OVERRIDE to copy the 'repair_line_id' field as well.
