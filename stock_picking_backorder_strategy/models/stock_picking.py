...
class StockPicking(models.Model):
    ...
    def _create_backorder(self, backorder_moves=None):
        ...
        pickings = self.filtered(lambda p: p.picking_type_id.backorder_strategy != 'no_create')
        pickings_no_create = self - pickings
        pickings_no_create.mapped('move_lines')._cancel_remaining_quantities()
        res = super(StockPicking, pickings)._create_backorder(backorder_moves=backorder_moves)
        to_cancel = res.filtered(lambda b: b.backorder_id.picking_type_id.backorder_strategy == 'cancel')
