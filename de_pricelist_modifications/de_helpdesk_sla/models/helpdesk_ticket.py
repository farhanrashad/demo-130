...
class HelpdeskTicket(models.Model):
    ...
    helpdesk_sla_id = fields.Many2one('helpdesk.ticket.sla', "Ticket SLA", compute="get_value_func",help="Only apply the SLA to a specific ticket type. If left empty it will apply to all types.")
    ...
    def get_value_func(self):
        if self:
            li=0
            for s in self:
                if s.status_sla_line:
                    for ss in s.status_sla_line:
                        if ss.status=="acheived":
                            li = self.env['helpdesk.ticket.sla'].search([('name','=',ss.name)])
                            if li:
                                s.helpdesk_sla_id=li.id
                        else:
                            s.helpdesk_sla_id=li
                else:
                    s.helpdesk_sla_id=''       
